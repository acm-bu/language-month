...
## Switch Statements
Switch statements look nearly identical in javascript to C++.
...
The break statements are necessary to avoid weird passthrough between cases which is pretty much never what you want to happen. You can also use early returns if applicable.
...
Statements familiar from almost all languages like `break` and `continue` will also work in javascript loops the same way you'd expect.

# Puzzle
For this first puzzle, create a calculator command line application. The program should read in lines from the console in the format `<number> <operator> <number>` where `<number>` can be any number and `<operator>` can be either `+`, `-`, `*`, or `/`. Below is the starter code which shows how to read lines from a console (since this requires `await`, which we haven't covered yet). You'll have to research and learn on your own:
...
// this is a "weird" for loop using a special AsyncIterator pattern. It's the
// most convinient way to read from the console in modern javascript
for await (const line of console) {
  // this will just echo whatever was last inputted
  console.log(line);
}
```

Once you finish, upload your code by pressing the submit button!
