...
We also now know that `console.log(string)` is the way to output text. `console` is a global object as defined [here](https://nodejs.org/api/console.html) (this is the node page because bun and node's console interfaces are nearly identical. Bun provides some [extra apis](https://bun.com/docs/api/console).). The `log` method is a method attatched to the global console object and we use the `.` operator to access it like we would on other languages.
